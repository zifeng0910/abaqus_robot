...
def main() -> None:
    ...
    selected = next(x for x in screen["cases"] if x["factor"] == args.factor)
    ...
    for stale in ("last_ODB_RP_time_ms", "last_online_time_ms", "run_started_at", "run_ended_at"):
    ...
    identity.update({"case_id": job, "source_case": SOURCE_NAME, "status": "PREPARED",
                     ...
                     "rotational_damping_screen_factor": args.factor,
