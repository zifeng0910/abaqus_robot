...
# Vector momentum balance is evaluated in global components; W0 is strictly
# before geometric contact (6.15 ms for this run).
...
for i in range(1,len(t)): J[i]=J[i-1]+.5*(F[i]+H[i]+F[i-1]+H[i-1])*.5*(t[i]-t[i-1])
rows=[]
for name,sel in [('W0_precontact',mask),('all',np.ones(len(t),bool))]:
