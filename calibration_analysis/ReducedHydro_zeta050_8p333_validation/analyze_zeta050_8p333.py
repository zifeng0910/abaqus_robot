...
def analyze_new_case():
    ...
    tail_ids, head_ids = all_ids[projection <= low], all_ids[projection >= high]
    tail0 = all_nodes[projection <= low].mean(axis=0); head0 = all_nodes[projection >= high].mean(axis=0)
    ...
    windows.to_csv(HERE / 'zeta050_8p333_phase_windows.csv', index=False)
    ...
    pd.DataFrame({'time_s': t, 'opposing_bridge': bridge_flag.astype(int),
                 ...
                 ).to_csv(HERE / 'zeta050_8p333_bridge_timeline.csv', index=False)
    ...
    event_catalog.to_csv(HERE / 'zeta050_8p333_contact_events.csv', index=False)
    ...
    phase_data.to_csv(HERE / 'zeta050_8p333_local_phase.csv', index=False)
    ...
    axis_data.to_csv(HERE / 'zeta050_8p333_true_axis.csv', index=False)
    ...
    torque_data.to_csv(HERE / 'zeta050_8p333_magnetic_hydro_torque.csv', index=False)
    ...
    translation.to_csv(HERE / 'zeta050_8p333_canonical_translation.csv', index=False)
    ...
    energy.to_csv(HERE / 'zeta050_8p333_energy.csv', index=False)
    ...
    (HERE / 'zeta050_8p333_summary.json').write_text(json.dumps(runtime, indent=2))
    pd.DataFrame([runtime]).drop(columns=['head_node_ids', 'tail_node_ids']).to_csv(
        HERE / 'zeta050_8p333_runtime_identity.csv', index=False)
