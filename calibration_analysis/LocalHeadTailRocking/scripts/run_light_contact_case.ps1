...
$out = Join-Path $repo 'calibration_analysis\LocalHeadTailRocking'
...
$amplitude = [string]::Format([Globalization.CultureInfo]::InvariantCulture, '{0:R}', [double]$identity.rocking_field_amplitude_deg)
...
if ([double]$identity.rocking_field_amplitude_deg -gt 20.0) { throw 'Selected amplitude exceeds 20 deg hard limit' }
...
$arguments = @('-u',$server,'--host','127.0.0.1','--port',"$Port",'--job-name',$job,
    '--dxf',$dxf,'--drive-type','analytic','--field-frame-mode','ROBOT_LOCAL_ROCKING',
    '--rocking-amplitude-deg',$amplitude,'--rocking-frame-azimuth-deg','-61.37284757596327','--robot-diameter-mm','0.815','--robot-height-mm','2.4',
    ...
    '--z-offset-mm','90','--driver-start-offset-mm','18.899960626','--spin-hz','5',
