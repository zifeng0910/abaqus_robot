[CmdletBinding()]
param([int]$Port = 65430)
$ErrorActionPreference = 'Stop'
$repo = 'J:\abaqusfangzhen\abaqus_robot'
$out = Join-Path $repo 'calibration_analysis\LocalPrecessingRocking'
$job = 'PROD_LOCAL_PRECESSROCK_10HZ_PREC2P5_G0_STRAIGHT'
$case = Join-Path $out ('case\' + $job)
$server = 'J:\magpy\magpylib_socket_server.py'
$vendored = Join-Path $repo 'calibration_analysis\ProductionLocalFrameValidation\production\magpylib_socket_server.py'
...
$vendoredHash = (Get-FileHash $vendored -Algorithm SHA256).Hash.ToLower()
$liveHash = (Get-FileHash $server -Algorithm SHA256).Hash.ToLower()
...
if ($identity.case_id -ne $job -or $identity.field_frame_mode -ne 'ROBOT_LOCAL_PRECESSING_ROCKING') { throw 'Case identity mismatch' }
if ([double]$identity.rocking_field_amplitude_deg -ne 14.343111711438091) { throw 'Amplitude drift' }
if ([double]$identity.rocking_frequency_Hz -ne 10 -or [double]$identity.precession_frequency_Hz -ne 2.5) { throw 'Frequency drift' }
if ([double]$identity.precession_phase_deg -ne -83.87284757596327) { throw 'Precession phase drift' }
...
$arguments = @('-u',$server,'--host','127.0.0.1','--port',"$Port",'--job-name',$job,
    '--dxf',$dxf,'--drive-type','analytic','--field-frame-mode','ROBOT_LOCAL_PRECESSING_ROCKING',
    '--rocking-amplitude-deg','14.343111711438091','--precession-frequency-hz','2.5','--precession-phase-deg','-83.87284757596327',
    ...
    '--z-offset-mm','90','--driver-start-offset-mm','18.899960626','--spin-hz','10',
