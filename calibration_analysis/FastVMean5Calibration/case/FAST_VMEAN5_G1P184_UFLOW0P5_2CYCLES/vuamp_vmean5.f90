...
contains
  subroutine load_magnetic_table()
    ...
    open(unit=91,file='J:\abaqusfangzhen\abaqus_robot\calibration_analysis\PrecomputedStraightMagneticBackend\magnetic_field_gradient_table.dat', &
  ...
  subroutine open_increment_log()
    if (log_open) return
    open(unit=77,file='J:\abaqusfangzhen\abaqus_robot\calibration_analysis\FastVMean5Calibration\case\FAST_VMEAN5_G1P184_UFLOW0P5_2CYCLES\magnetic_hydro_increment.csv',status='replace')
    write(77,'(A)') 'time_s,s_eff_mm,phase_deg,Fmag1_N,Fmag2_N,Fmag3_N,Mmag1_Nmm,Mmag2_Nmm,Mmag3_Nmm,Fhydro1_N,Fhydro2_N,Fhydro3_N,Mhydro1_Nmm,Mhydro2_Nmm,Mhydro3_Nmm,v_s_mm_s,rocking_rate_rad_s'
    log_open=.true.
  end subroutine open_increment_log
end module fast_precomputed_data
...
subroutine vuamp(ampName,time,ampValueOld,dt,nprops,props,nSvars,svars, &
  ...
  real(8) :: loads(6),hloads(6),last_time,t,u(3),ur(3),vel(3),omega(3)
  real(8) :: c(3),a0(3),a(3),m0(3),moment(3),flow(3),vrel(3),vside(3),wspin(3),wwob(3)
  real(8) :: b(3),grad(3,3),ang,kx,ky,kz,cc,ss,q,scale,s_eff,phase_deg,dotu,vpar,wpar
  real(8) :: cpar,cperp,kspin,kwob,v_s,wrock
  integer :: initialized,idx,i,j,is_master
  logical :: outside
  double precision VGETSENSORVALUE
  save loads,hloads,last_time,initialized
  data initialized /0/
  ...
  data flow /0.4881399801232148d0,-0.0309160123723488d0,0.1037475782093262d0/
  data cpar /4.0d-9/,cperp /1.2d-8/,kspin /1.0d-9/,kwob /3.0d-9/
  ...
  if (initialized==0) then
    loads=0.0d0; hloads=0.0d0; last_time=-1.0d99; initialized=1
  endif
  is_master=1
  ...
  if (is_master==1 .and. abs(t-last_time)>1.0d-15) then
    ...
    vel(1)=VGETSENSORVALUE('RP_V1',jSensorLookUpTable,sensorValues)
    vel(2)=VGETSENSORVALUE('RP_V2',jSensorLookUpTable,sensorValues)
    vel(3)=VGETSENSORVALUE('RP_V3',jSensorLookUpTable,sensorValues)
    omega(1)=VGETSENSORVALUE('RP_VR1',jSensorLookUpTable,sensorValues)
    omega(2)=VGETSENSORVALUE('RP_VR2',jSensorLookUpTable,sensorValues)
    omega(3)=VGETSENSORVALUE('RP_VR3',jSensorLookUpTable,sensorValues)
    dotu=dot_product(u,c)
    s_eff=34.899921242759286d0+dotu-6.0d0*t
    ...
    b=0.010d0*b; grad=0.001184d0*grad
    if (t<0.001d0) then
      q=max(0.0d0,min(1.0d0,t/0.001d0)); scale=q*q*(3.0d0-2.0d0*q)
    else
      scale=1.0d0
    endif
    do i=1,3
    ...
    vrel=vel-flow
    vpar=dot_product(vrel,a); vside=vrel-vpar*a
    hloads(1:3)=-cpar*vpar*a-cperp*vside
    wpar=dot_product(omega,a); wspin=wpar*a; wwob=omega-wspin
    hloads(4:6)=-kspin*wspin-kwob*wwob
    v_s=dot_product(vel,c); wrock=sqrt(max(0.0d0,dot_product(wwob,wwob)))
    call open_increment_log()
    write(77,'(17(ES18.10,:,","))') t,s_eff,phase_deg,loads,hloads,v_s,wrock
    last_time=t
  endif
  AmpValueNew=0.0d0
  ...
  if (ampName(1:8)=='HYDRO_FX') AmpValueNew=hloads(1)
  if (ampName(1:8)=='HYDRO_FY') AmpValueNew=hloads(2)
  if (ampName(1:8)=='HYDRO_FZ') AmpValueNew=hloads(3)
  if (ampName(1:8)=='HYDRO_MX') AmpValueNew=hloads(4)
  if (ampName(1:8)=='HYDRO_MY') AmpValueNew=hloads(5)
  if (ampName(1:8)=='HYDRO_MZ') AmpValueNew=hloads(6)
  return
