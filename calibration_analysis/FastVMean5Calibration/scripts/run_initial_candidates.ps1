...
foreach ($job in $jobs) {
    ...
    $identity.status = 'SOLVED'; $identity.dynamics_run_count = 1
    ...
    $results += [ordered]@{job=$job; G_mT=$identity.gradient_mT; wallclock_s=$wallclock; datacheck_passed=$true}
    Write-Host ("COMPLETE {0}; wallclock={1:N1}s" -f $job,$wallclock)
}
$results | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $out 'initial_run_status.json') -Encoding ascii
