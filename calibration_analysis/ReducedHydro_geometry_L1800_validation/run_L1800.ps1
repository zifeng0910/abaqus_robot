[CmdletBinding()]
param([string]$WorkDir='J:\abaqusfangzhen',[int]$Port=65509)
...
$identity=Get-Content -Raw 'J:\abaqusfangzhen\abaqus_robot\calibration_analysis\ReducedHydro_geometry_L1800_validation\L1800_input_identity.json' | ConvertFrom-Json
$moment=[string]::Format([Globalization.CultureInfo]::InvariantCulture,'{0:R}',[double]$identity.candidate_magnetic_moment_Am2)
$mass=[string]::Format([Globalization.CultureInfo]::InvariantCulture,'{0:R}',[double]$identity.candidate_mass_mg)
...
$diameter=[string]::Format([Globalization.CultureInfo]::InvariantCulture,'{0:R}',[double]$identity.candidate_pca_transverse_diameter_mm)
$args=@('-u',$server,'--host','127.0.0.1','--port',"$Port",'--job-name',$Job,'--dxf','J:\magpy\curvenew_CEL_xyrot56_exact.dxf','--drive-type','analytic','--robot-diameter-mm',$diameter,'--robot-height-mm','1.8','--robot-br-t','1.46','--robot-moment-Am2',$moment,'--robot-mass-mg',$mass,'--analytic-b-t','0.010','--analytic-follow-robot','--analytic-gradient-b-t','0.006','--analytic-gradient-length-mm','45','--analytic-gradient-profile','legacy','--analytic-gradient-switch-start-s','0','--analytic-gradient-transition-s','0.0005','--driver-speed-mm-s','6','--bend-speed-mm-s','4.5','--bend-start-mm','13.49','--bend-end-mm','18.56','--z-offset-mm','90','--driver-start-offset-mm','18.899960626','--spin-hz','30','--cone-half-angle-deg','30','--driver-xy-shift-mm','2','-6','--cone-axis-bias-deg','40','--cone-axis','tangent','--robot-polarity','-1','--robot-axis-tangent','--phase-deg','248','--analytic-rotation-sense','1','--ramp-time-s','0.001','--endpoint-taper-mm','1','--adaptive-lead-mm','0','--frame-transform-json',$transform,'--telemetry-csv',$telemetry,'--telemetry-interval-s','0.0','--frame-log-interval-s','0.00005')
Write-Host "GEOMETRY CANDIDATE: L=1.800 mm, D=$diameter mm, mass=$mass mg, moment=$moment A m2"
Write-Host 'FROZEN: zeta=0.50, tangent=0, SmoothWall114, Reduced-Hydro coefficients and magnetic field parameters'
