"""Prepare and gate the single F60 background-flow screening case."""
...
OUT = HERE.parent
REPO = OUT.parents[1]
CASE = OUT / "case" / "S4_HEADFORWARD_F60_G0P10_FLOW"
JOB = "S4_HEADFORWARD_F60_G0P10_FLOW"
SOURCE_CASE = REPO / "calibration_analysis" / "F40LowGBackgroundFlowScreen" / "case" / "S4_HEADFORWARD_F40_G0P05_FLOW"
SOURCE_ID = SOURCE_CASE / "case_identity.json"
...
def sign_gates(source):
    ...
    d_b_ds = -(delta / 45.0**2) * math.exp(-0.5 * (delta / 45.0) ** 2) * 0.10e-3 * 1000.0
...
def main():
    ...
    if "2.0e-7, 0.037500000" not in deck_text:
        raise RuntimeError("F60 deck does not preserve the requested dt/duration")
    ...
    identity = {
        ...
        "source_case": source["case_id"], "source_commit": "24aec94",
        "field_frame_mode": "ROBOT_LOCAL_ELLIPTIC_ROCKING", "duration_s": 0.0375,
        "direct_dt_s": 2.0e-7, "frequency_Hz": 60.0,
        ...
        "B0_mT": source["B0_mT"], "gradient_mT": 0.10, "gradient_length_mm": 45.0,
        ...
        "source_input_sha256": sha256(SOURCE_CASE / "S4_HEADFORWARD_F40_G0P05_FLOW.inp"),
