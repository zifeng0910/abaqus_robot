"""Abaqus-Python extraction of dense RP, contact, and energy histories."""
...
for region_name, region in step.historyRegions.items():
    for variable, history in region.historyOutputs.items():
        ...
        if variable in [prefix + str(axis) for prefix in ("U", "UR", "V", "VR") for axis in (1, 2, 3)]:
            current = rp.get(variable)
            if current is None or len(array) > len(current):
                rp[variable] = array
        if variable.startswith(("CFN", "CFS", "CFT")):
            contact[region_name + "|" + variable] = array
...
np.savez_compressed(out / "energy_history_private.npz", **energy)
