...
$validation = Join-Path $repo 'calibration_analysis\ProductionLocalFrameValidation'
$case = Join-Path $validation 'case\PROD_LOCAL30_G0_3CYCLE'
...
$vendoredHash = (Get-FileHash (Join-Path $validation 'production\magpylib_socket_server.py') -Algorithm SHA256).Hash
...
$arguments = @('-u',$server,'--host','127.0.0.1','--port',"$Port",'--job-name',$job,
    ...
    '--field-frame-mode','ROBOT_LOCAL_TANGENT','--robot-diameter-mm','0.815','--robot-height-mm','2.4',
