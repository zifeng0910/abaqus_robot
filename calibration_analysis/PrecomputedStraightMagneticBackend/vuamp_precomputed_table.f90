module precomputed_magnetic_data
  implicit none
  integer, parameter :: max_s = 2001, max_phase = 721
  integer :: ns = 0, nphase = 0
...
contains
  subroutine load_magnetic_table()
    ...
    if (ios /= 0) then
      write(6,*) 'PRECOMPUTED_TABLE_FATAL: cannot open magnetic table, iostat=', ios
      call xplb_abqerr(-3,'PRECOMPUTED_TABLE_FATAL: missing magnetic table',0,0.0d0,' ')
      return
    endif
    read(91,*,iostat=ios) ns, nphase
    if (ios /= 0 .or. ns < 2 .or. ns > max_s .or. nphase < 2 .or. nphase > max_phase) then
      write(6,*) 'PRECOMPUTED_TABLE_FATAL: invalid dimensions ',ns,nphase
      call xplb_abqerr(-3,'PRECOMPUTED_TABLE_FATAL: invalid dimensions',0,0.0d0,' ')
      return
    endif
    do i=1,nphase
...
end module precomputed_magnetic_data
...
subroutine vuamp(ampName,time,ampValueOld,dt,nprops,props,nSvars,svars, &
  ...
  real(8) :: loads(6), last_time, t, u(3), ur(3), c(3), m0(3), moment(3)
  real(8) :: b(3), grad(3,3), ang, kx, ky, kz, cc, ss, q, scale
  real(8) :: s_eff, phase_deg, dotu
  ...
  if (is_master == 1 .and. abs(t-last_time) > 1.0d-15) then
    ...
    ang=sqrt(ur(1)**2+ur(2)**2+ur(3)**2)
    if (ang < 1.0d-14) then
      moment=m0
    else
      kx=ur(1)/ang; ky=ur(2)/ang; kz=ur(3)/ang
    ...
    do i=1,3
      loads(i)=scale*(moment(1)*grad(1,i)+moment(2)*grad(2,i)+moment(3)*grad(3,i))
    enddo
