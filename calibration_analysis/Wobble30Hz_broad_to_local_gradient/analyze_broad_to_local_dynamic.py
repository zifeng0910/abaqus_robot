...
summary.to_csv(ROOT / 'broad_to_local_8p333ms_canonical_motion.csv', index=False)
...
win.to_csv(ROOT / 'broad_to_local_8p333ms_velocity_windows.csv', index=False)
...
pd.DataFrame(budget).to_csv(ROOT / 'broad_to_local_8p333ms_momentum_budget.csv', index=False)
...
pd.DataFrame(events).to_csv(ROOT / 'broad_to_local_8p333ms_contact.csv', index=False)
summary[['case','UR1_pp_rad','UR2_pp_rad','UR3_pp_rad','max_abs_axis_angle_deg']].to_csv(ROOT / 'broad_to_local_8p333ms_wobble.csv', index=False)

colors = {'G6_L45':'#377eb8', 'Gated_G6_L45_to_L3p75':'#e41a1c'}
...
report_path = ROOT/'Wobble30Hz_broad_to_local_gradient_report.md'
if not report_path.exists(): report_path.write_text('# 30 Hz broad-to-local gradient report\n', encoding='utf-8')
...
with open(report_path,'w',encoding='utf-8') as f:
    ...
    f.write('The production broad-to-local profile was tested in the Wall-ON CEL model with no other physical parameter change.\n\n')
    f.write(md_table(summary)); f.write('\n\n')
    f.write('The gated profile ends at **Δs = %.4f mm** and **Vt = %.1f mm/s**, while G6/L45 ends at **Δs = %.4f mm** and **Vt = %.1f mm/s**. Candidate CPRESS peak is %.2f MPa and exact minimum signed gap %.3f µm.\n\n' % (r.loc['Gated_G6_L45_to_L3p75','delta_s_mm'],r.loc['Gated_G6_L45_to_L3p75','Vt_final_mm_s'],r.loc['G6_L45','delta_s_mm'],r.loc['G6_L45','Vt_final_mm_s'],r.loc['Gated_G6_L45_to_L3p75','CPRESS_max_MPa'],r.loc['Gated_G6_L45_to_L3p75','min_gap_um']))
    f.write('### Stage-wise canonical velocity\n\n'+md_table(win)+'\n\n')
    f.write('The gated profile delays the strong local force until after the broad-contact phase. UR1 peak-to-peak is %.3f rad.\n\n' % r.loc['Gated_G6_L45_to_L3p75','UR1_pp_rad'])
    f.write('**Decision:** see the quantitative gates above; no continuation is automated before review.\n')
