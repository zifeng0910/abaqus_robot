[CmdletBinding()]
param([string]$WorkDir='J:\abaqusfangzhen',[string]$JobName='WobbleCal_F30_Cone30_B10_WallOn_Free_003',[int]$Port=65495,[double]$GradientT=0.003,[double]$GradientLengthMm=45.0,[string]$GradientProfile='legacy',[double]$GradientSwitchStartS=0.0,[double]$GradientTransitionS=0.0005,[double]$GradientLocalT=-1.0,[double]$GradientLocalLengthMm=3.75)
...
$lengthText = $GradientLengthMm.ToString('0.#########',[Globalization.CultureInfo]::InvariantCulture)
$switchText = $GradientSwitchStartS.ToString('0.#########',[Globalization.CultureInfo]::InvariantCulture)
$transitionText = $GradientTransitionS.ToString('0.#########',[Globalization.CultureInfo]::InvariantCulture)
$localLengthText = $GradientLocalLengthMm.ToString('0.#########',[Globalization.CultureInfo]::InvariantCulture)
$args=@($server,'--host','127.0.0.1','--port',"$Port",'--job-name',$JobName,'--dxf','J:\magpy\curvenew_CEL_xyrot56_exact.dxf','--drive-type','analytic','--robot-diameter-mm','1.22','--robot-height-mm','2.81','--robot-br-t','1.46','--robot-moment-Am2','0.001168','--robot-mass-mg','10','--analytic-b-t','0.010','--analytic-follow-robot','--analytic-gradient-b-t',$gradientText,'--analytic-gradient-length-mm',$lengthText,'--analytic-gradient-profile',$GradientProfile,'--analytic-gradient-switch-start-s',$switchText,'--analytic-gradient-transition-s',$transitionText,'--analytic-gradient-local-length-mm',$localLengthText,'--driver-speed-mm-s','6','--bend-speed-mm-s','4.5','--bend-start-mm','13.49','--bend-end-mm','18.56','--z-offset-mm','90','--driver-start-offset-mm','18.899960626','--spin-hz','30','--cone-half-angle-deg','30','--driver-xy-shift-mm','2','-6','--cone-axis-bias-deg','40','--cone-axis','tangent','--robot-polarity','-1','--robot-axis-tangent','--phase-deg','248','--analytic-rotation-sense','1','--ramp-time-s','0.001','--endpoint-taper-mm','1','--adaptive-lead-mm','0','--frame-transform-json',$transform,'--telemetry-csv',$telemetry,'--telemetry-interval-s','0.0001','--frame-log-interval-s','0.0001')
if($GradientLocalT -ge 0){$args += @('--analytic-gradient-local-b-t',($GradientLocalT.ToString('0.#########',[Globalization.CultureInfo]::InvariantCulture)))}
$py='I:\SIMULIA\EstProducts\2025\win_b64\tools\SMApy\python3.10\python.exe'; if(-not(Test-Path $py)){$py='python'}
Write-Host "Starting analytic field server: F=30 Hz, B=10 mT, gradient=$gradientText T, profile=$GradientProfile, switch=$switchText s, transition=$transitionText s, WALL-ON, free translation, port $Port"
