...
if ($identity.status -ne 'PREPARED' -or -not $identity.forward_sign_sanity.passed) { throw 'Preparation/sign gate failed' }
. (Join-Path (Split-Path $repo -Parent) 'launch_abaqus_with_env.ps1') -SkipVerify
$ErrorActionPreference = 'Continue'
Set-Location -LiteralPath $case
$sub = Join-Path $case 'vuamp_precomputed_g1p20_flow.f90'
