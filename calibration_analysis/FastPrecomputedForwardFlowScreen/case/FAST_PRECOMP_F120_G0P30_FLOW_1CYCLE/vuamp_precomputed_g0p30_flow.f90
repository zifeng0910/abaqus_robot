...
contains
  ...
  subroutine open_increment_log()
    if (log_open) return
    open(unit=77,file='J:\abaqusfangzhen\abaqus_robot\calibration_analysis\FastPrecomputedForwardFlowScreen\case\FAST_PRECOMP_F120_G0P30_FLOW_1CYCLE\magnetic_hydro_increment.csv',status='replace')
...
subroutine vuamp(ampName,time,ampValueOld,dt,nprops,props,nSvars,svars, &
  ...
  if (is_master==1 .and. abs(t-last_time)>1.0d-15) then
    ...
    b=0.010d0*b; grad=0.00030d0*grad
    if (t<0.001d0) then
