...
$out = Join-Path $repo 'calibration_analysis\S4HeadForwardLowGScreen'
...
function Get-Sha256([string]$Path) {
    ...
    try { $sha = [System.Security.Cryptography.SHA256]::Create(); try { return ([BitConverter]::ToString($sha.ComputeHash($stream))).Replace('-','').ToLower() } finally { $sha.Dispose() } }
    finally { $stream.Dispose() }
}
if ((Get-Sha256 $server) -ne $identity.fast_server_sha256) { throw 'Fast server identity mismatch' }
...
$arguments = @('-u',$server,'--host','127.0.0.1','--port',"$Port",'--job-name',$Job,
    ...
    '--analytic-b-t','0.010','--analytic-follow-robot','--analytic-gradient-b-t',([string]($identity.gradient_mT / 1000.0)),
...
} finally { if ($process -and -not $process.HasExited) { try { $process.Kill() } catch {} } }
