"""Lightweight analysis and fixed-camera rendering for the one F40 screen."""
...
def render_animation(time, data, nodes, region, identity, c, n, rp0, rocking, axial, path, step_s, fps):
    ...
    def draw(frame):
        ...
        ax.text(0.095, 0.94, "TAIL", color="#2864a8", weight="bold", transform=ax.transAxes, va="top")
        ax.set_title(
            f"{JOB} | t={time[index] * 1e3:5.1f} ms | f=40 Hz | G=0.05 mT | U_flow=+10 mm/s | "
...
def main():
    ...
    left.set_title("F40 background-flow screening: magnetic / ReducedHydro / axial velocity")
    ...
    report = f"""# F40 Low-G Background-Flow Screen
...
The force/velocity plot is `{JOB}_force_velocity.png`. The slow contact-inspection GIF is `{JOB}_SLOW.gif`; the faster playback is `{JOB}_FAST.mp4` when an ffmpeg encoder is available (`fast_mp4_status={fast_mp4_status}`). This result is a coarse visual precursor to later true fluid simulation, not a publication-grade FSI result.
