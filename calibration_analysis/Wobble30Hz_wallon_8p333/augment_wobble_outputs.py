...
# fixed-camera GIF
tri=pd.read_csv(ROOT/(JOB+'_pipe_wall_triangles_exact.csv')); wn=pd.read_csv(ROOT/(JOB+'_pipe_wall_nodes_exact.csv')); nd={int(r.node):[r.x_mm,r.y_mm,r.z_mm] for _,r in wn.iterrows()}; verts=[[nd[int(r[n])] for n in ('n1','n2','n3','n4')] for _,r in tri.iterrows()]; axraw=pd.read_csv(ROOT/(JOB+'_true_axis_raw.csv')); curve=pd.read_csv(ROOT/'curvenew_CEL_xyrot56_exact.csv'); ims=[]
for k in np.linspace(0,len(t)-1,min(84,len(t))).astype(int):
    fig=plt.figure(figsize=(7,5)); ax=fig.add_subplot(111,projection='3d'); ax.add_collection3d(Poly3DCollection(verts,alpha=.10,facecolor='gray',edgecolor='none')); ax.plot(curve.x_mm,curve.y_mm,curve.z_mm,'k-',lw=1,alpha=.7)
    ...
    ax.set_xlim(curve.x_mm.min(),curve.x_mm.max()); ax.set_ylim(curve.y_mm.min(),curve.y_mm.max()); ax.set_zlim(curve.z_mm.min(),curve.z_mm.max()); ax.view_init(18,-60); ax.set_title('t=%.2f ms | phase-rate=%.1f Hz | |wobble|=%.0f rad/s'%(t[k]*1000,d.robot_phase_equiv_Hz.iloc[k],d.omega_wobble_rad_s.iloc[k])); fig.canvas.draw(); ims.append(Image.frombytes('RGB',fig.canvas.get_width_height(),fig.canvas.tostring_rgb())); plt.close(fig)
