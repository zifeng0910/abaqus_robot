...
do {
  Clear-Host; $stamp=Get-Date -Format 'yyyy-MM-dd HH:mm:ss'; Write-Host "=== COM-fixed wobble calibration monitor ===" -ForegroundColor Cyan; Write-Host "[$stamp] $JobName"
  ...
  if($done){Write-Host 'Abaqus: COMPLETED SUCCESSFULLY' -ForegroundColor Green}elseif($proc){Write-Host 'Abaqus: RUNNING' -ForegroundColor Green}elseif(Test-Path $lck){Write-Host 'Abaqus: LOCK PRESENT / PROCESS NOT VISIBLE' -ForegroundColor Yellow}else{Write-Host 'Abaqus: NOT FOUND or STOPPED' -ForegroundColor Yellow}
